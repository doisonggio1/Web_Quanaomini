...
CREATE TABLE IF NOT EXISTS `coupon` ( -- liên kết một nhiều với catalog
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `catalog_id` INT(11) DEFAULT NULL, -- cho phép null
  `type_coupon` INT(11) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `transaction` (
  ...
  `amount` DECIMAL(15,2) NOT NULL DEFAULT 0.00, -- Tổng tiền đơn hàng
...
CREATE TABLE IF NOT EXISTS `order` (
  ...
  `amount` DECIMAL(15,2) NOT NULL DEFAULT 0.00, -- Tổng tiền hàng (chỉ có hàng thôi)
  `status` INT(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
...
('Azura', 'lam@gmail.com', '1bbd886460827015e5d605ed44252251', 0, '2025-06-18 05:59:50'),
('Azura', 'phannguyen2300@gmail.com', '1bbd886460827015e5d605ed44252251', 0, '2025-06-18 05:59:50');
...
('Phan Nguyên', 'phannguyen2300@gmail.com', '1bbd886460827015e5d605ed44252251', '0908765432', 'Đà Nẵng', 'Đà Nẵng', 'Hải Châu', 'Thạch Thang', NOW(), 1, NOW());

-- nếu type_coupon == 0: voucher, type_coupon == 1: gift code 
-- privilege: quyền sử dụng: 0 là private, 1 là public
-- nếu type == 0: phí ship, type == 1: sale (giảm giá hàng)
-- nếu value <= 100: đơn vị là %, nếu > 100: đơn vị là VND, nếu value == 0 thì là miễn phí
-- nếu min_price != null: giảm giá cho đơn từ xx VND, min_price == null: giảm giá toàn bộ sản phẩm
-- nếu max_value != null: giảm tối đa xx VND, max_value == null, không có giới hạn tối đa
-- catalog_id == xx: loại mặt hàng áp dụng, catalog_id == null: áp dụng cho tất cả ngành hàng
-- usage_limit: số lần dùng tối đa của voucher cho một user, usage_limit == null: không giới hạn số lượng
-- total_quantity: tổng số lượng voucher trên hệ thống, total_quantity: == null: không giới hạn tổng số lượng 
...
VALUES
-- FREESHIP0 (id = 1): user 2 đã dùng 2 lần
(2, 1, 2, DATE_SUB(NOW(), INTERVAL 3 DAY)),

-- SHIP30K (id = 2): user 3 chưa dùng
(3, 2, 0, '2025-01-01 00:00:00'),

-- SALE15P (id = 3): user 1 đã dùng 1 lần
(1, 3, 1, DATE_SUB(NOW(), INTERVAL 1 DAY)),

-- SALE60K (id = 4): user 4 chưa dùng
(4, 4, 0, '2025-01-01 00:00:00'),

-- SALE5P_ALL (id = 5): user 3 đã dùng 3 lần
(3, 5, 3, DATE_SUB(NOW(), INTERVAL 6 DAY)),

-- SALE40K (id = 6): user 1 chưa dùng
(1, 6, 0, '2025-01-01 00:00:00'),

-- SALE20P_NO_LIMIT (id = 7): user 2 đã dùng 1 lần
(2, 7, 1, DATE_SUB(NOW(), INTERVAL 2 DAY)),

-- SALE25P_LIMIT (id = 8): user 4 chưa dùng
(4, 8, 0, '2025-01-01 00:00:00'),

-- ALLCAT10P (id = 9): user 1 đã dùng 2 lần
(1, 9, 2, DATE_SUB(NOW(), INTERVAL 4 DAY)),

-- CAT13-20K (id = 10): user 3 đã dùng 1 lần
(3, 10, 1, DATE_SUB(NOW(), INTERVAL 1 DAY)),

-- User 4 dùng thêm voucher id 1, 2, 5
(4, 1, 1, DATE_SUB(NOW(), INTERVAL 1 DAY)),
(4, 2, 2, DATE_SUB(NOW(), INTERVAL 5 DAY)),
(4, 5, 1, DATE_SUB(NOW(), INTERVAL 2 DAY)),

-- User 4 dùng voucher id 9 và 10 nhiều lần
...
(0, 3, 'Tô Nam', 'tonam@yahoo.com.vn', '098989876', 'Thủy Nguyên - Hải Phòng', 'Hải Phòng', 'Thủy Nguyên', 'Minh Đức', 'Ship đến địa chỉ vào sáng ngày 23/5', '469000.00', '', '2017-05-09 01:07:54'),
(1, 1, 'GoO', 'GoO@gmail.com', '01215345336', 'Hải Phòng', 'Hải Phòng', 'Lê Chân', 'Dư Hàng', 'Ship vào sáng mai.', '70000.00', '', '2017-05-08 19:47:54'),
(0, 2, 'Linh', 'ling@yahoo.com', '098798787', 'hai Phong', 'Hải Phòng', 'Hồng Bàng', 'Quang Trung', 'ship', '69000.00', '', '2017-05-09 19:11:14'),
(1, 3, 'Nhi', 'nhi@test.com', '0987654321', 'Long Biên - Hà Nội', 'Hà Nội', 'Long Biên', 'Ngọc Lâm', 'Gửi hàng đến địa chỉ trên vào ngày mai', '200000.00', '', '2017-05-05 10:47:54'),
(0, 1, 'VIP User', 'test@gmail.com', '1234567890', 'Hải Phòng', 'Hải Phòng', 'Lê Chân', 'An Biên', 'Ship free', '450000.00', '', '2017-05-05 10:47:54'),
(0, 2, 'test', 'test@gmail.com', '1234567890', 'Hải Phòng', 'Hải Phòng', 'Hồng Bàng', 'Phạm Hồng Thái', 'TESE', '300000.00', '', '2017-05-10 01:34:34'),
(0, 3, 'Nguyen An', 'khachhang1@gmail.com', '01201212222', 'Thủy Nguyên - Hải Phòng', 'Hải Phòng', 'Thủy Nguyên', 'Quảng Thanh', 'SHIP TO', '169000.00', '', '2017-05-10 08:29:13');
...
(1, 23, '37693cfc748049e45d87b8c7d8b9aacd', 'COMBƯO ĐẦM REN MÙA XUÂN', 370000.00, 1, '', 'combo-dam-ren-mua-xuan-cho-me-va-be-th08602-gs210-1m4G3-g4rMfx.jpg', '2025-04-16 10:34:06', '2025-04-16 10:34:06'),
